...
for imgId in range(0,len(imgIds),2):
    ...
    print("Left image shape {} and {}".format(left_img.shape[0],left_img.shape[1]))
    print("Right image shape {} and {}".format(right_img.shape[0],left_img.shape[1]))
    image = {"date_captured":"2020","file_name":str(saveId)+".jpg","id":saveId,"license":1,"url":"","height":im.shape[0],"width":im.shape[1]}
    images.append(image)
    
    image = {"date_captured":"2020","file_name":str(saveId)+".jpg","id":saveId+1,"license":1,"url":"","height":im.shape[0],"width":im.shape[1]}
    images.append(image)
    for ann in anns:
        x =(ann['bbox'][0])
