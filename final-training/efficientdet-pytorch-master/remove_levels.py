...
parser.add_argument('--remove-differential-learning',action='store_true',default=False,help='train entire network')
parser.add_argument('--initial-checkpoint', default='', type=str, metavar='PATH',
                    help='Initialize model from this checkpoint (default: none)')
parser.add_argument('--my-checkpoint',default='',type=str,metavar='PATH',help='Load previous checkpoint')
...
def main():
    ...
    if args.my_checkpoint:
        state_dict = load_state_dict(args.my_checkpoint)
        model.load_state_dict(state_dict,strict=True) 
    if args.initial_checkpoint:
        full_state_dict = load_state_dict(args.initial_checkpoint)
        keys =list(full_state_dict.keys())
        for key in keys:
            if 'class_net.predict' in key:
    ...
    if args.remove_differential_learning:
        print("Removing differrential learning")
        optimizer=create_optimizer(args,model,remove_diff=True)
    else:
        optimizer = create_optimizer(args, model,remove_diff=False)
    use_amp = False
    ...
    train_annotation_path = os.path.join(args.data, 'annotations', f'train_annotations.json')
    ...
    train_anno_set = 'valid'
    train_annotation_path = os.path.join(args.data, 'annotations', f'valid_annotations.json')
    train_image_dir = train_anno_set
    dataset_eval = CocoDetection(os.path.join(args.data, train_image_dir), train_annotation_path)
    validation_mul = args.validation_batch_size_multiplier

    loader_eval = create_loader(
        dataset_eval,
        input_size=config.image_size,
        batch_size=args.batch_size * validation_mul,
        is_training=False,
        use_prefetcher=args.prefetcher,
