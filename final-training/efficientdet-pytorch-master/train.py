...
def main():
    ...
    model.class_net.predict.conv_pw =create_conv2d(config.fpn_channels,9*3,1,padding=config.pad_type,bias=True)
    variance_scaling(model.class_net.predict.conv_pw.weight)
    variance_scaling(model.class_net.predict.conv_dw.weight,groups = model.class_net.predict.conv_dw.groups)
    model.class_net.predict.conv_pw.bias.data.fill_(-math.log((1 - 0.01)/0.01))
    print(model.class_net.predict.conv_pw)
    model.box_net.predict.conv_pw =create_conv2d(config.fpn_channels,9*4,1,padding=config.pad_type,bias=True)
    variance_scaling(model.box_net.predict.conv_pw.weight)
    variance_scaling(model.box_net.predict.conv_dw.weight,groups=model.box_net.predict.conv_dw.groups)
    model.box_net.predict.conv_pw.bias.data.zero_()
    print(model.box_net.predict.conv_pw)
