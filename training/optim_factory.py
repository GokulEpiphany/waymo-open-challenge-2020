...
try:
    ...
    has_apex = False
def unfrozen_params(model):
    print("Using only unfrozen parameters")
    params = []
...
def add_weight_decay(model, weight_decay=1e-5, skip_list=()):
    ...
    for name, param in model.named_parameters():
        ...
        if len(param.shape) == 1 or name.endswith(".bias") or name in skip_list:
            ...
            decay.append(param)
    params_count = sum([np.prod(p.size()) for p in decay])
    params_count += sum([np.prod(p.size()) for p in no_decay])
    print(params_count)
    return [
